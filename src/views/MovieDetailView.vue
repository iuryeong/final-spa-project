...
const route = useRoute();    // useRoute 주소 파라미터 추출용
...
const goBack = () => {
  router.back();   // 브라우저 히스토리 스택 되돌려 목록 스크롤 위치 보존
};
...
<template>
  <main v-if="store.selectedMovie" class="detail-page">
    <div v-if="store.selectedMovie?.backdrop_path" 
    ...
    <div v-else class="backdrop-layer no-image">
      <div class="black-curtain"></div>
    </div>
      ...
      <div class="movie-hero-grid">
        ...
        <div class="info-zone">
          ...
          <div class="genres-wrapper">
            <span v-for="genre in store.selectedMovie?.genres" 
                  :key="genre.id" 
                  class="genre-tag">
              {{ genre.name }}
            </span>
  ...
  <div v-else-if="store.errorMessage" class="full-screen-error-gate">
    ...
    <h2 class="error-title">죄송합니다! 오류가 발생했습니다</h2>
...
.poster-zone { width: 100%; }
.main-poster { width: 100%; height: auto; border-radius: 20px; box-shadow: 0 15px 40px rgba(0,0,0,0.7); border: 1px solid rgba(255,255,255,0.15); display: block; }
...
.ai-section { margin: 30px 0; padding: 20px; border-radius: 12px; background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255,255,255,0.2); }
.ai-btn { background: linear-gradient(45deg, #8a2be2, #4b0082); color: white; border: none; padding: 12px 20px; font-size: 16px; font-weight: bold; border-radius: 8px; cursor: pointer; width: 100%; }
.ai-btn:hover:not(:disabled) { opacity: 0.9; }
.ai-btn:disabled { opacity: 0.6; cursor: not-allowed; }
.ai-result-box { margin-top: 15px; padding: 15px; background: #222; border-left: 4px solid #8a2be2; color: #fff; line-height: 1.6; }
.ai-result-box p { margin: 0; }
